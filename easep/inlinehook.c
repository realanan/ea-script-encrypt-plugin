...
void IH_Init(FARPROC *origFuncs, size_t funcCount, FARPROC *newFuncs)
{
	...
	for (i = 0; i < funcCount; i++)
	{
		...
		*(DWORD*)(ih_data[i].newData + 1) = (DWORD)newFuncs[i] - (DWORD)origFuncs[i] - 5;
		...
		VirtualProtect(ih_data[i].funcAddr, sizeof(ih_data[i].newData), PAGE_EXECUTE_READWRITE, &dwOldProtect);
...
void IH_HookOnAll()
{
	...
	for (i = 0; ih_data[i].funcAddr; i++) IH_HookOn(i);
}
...
void IH_HookOffAll()
{
	...
	for (i = 0; ih_data[i].funcAddr; i++) IH_HookOff(i);
}
